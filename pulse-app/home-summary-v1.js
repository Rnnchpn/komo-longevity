...
function render(){if(route()!=='home')return;const card=document.querySelector('.hero-grid .side-summary');if(!card)return;const name=displayName();card.classList.add('khs-card');card.innerHTML=`<div class="khs-head"><div class="khs-person"><div class="khs-avatar">${esc(initials(name))}</div><div><p class="eyebrow">VOTRE SYNTHÈSE</p><h3>${esc(name)}</h3><span>${esc(S.profile?.city||'Dossier KŌMØ')}</span></div></div></div><div class="khs-stack">${freeRow()}${motionRow()}${clinicalRow()}</div><div class="khs-foot"><div><span>Votre trajectoire</span><strong>${S.free?'Première référence enregistrée':'À commencer'}</strong></div><button type="button" data-route="${S.motionScore?'path':'documents'}">${S.motionScore?'Voir My KŌMØ Score':'Continuer avec Motion'} →</button></div>`}
