...
function currentScore() {
  ...
  if (score) return {
    ...
    confidence: score.confidence_label || score.confidence || null
  };
  const old = data.oldScores[0];
  if (!old) return null;
  return {
...
function normalizedDomains(raw) {
  ...
  return Object.entries(raw).map(([key, value]) => {
    ...
    return [labels[key] || key.replaceAll('_',' '), numberOrNull(numeric)];
...
async function loadMemberData(force = false) {
  ...
  if (!force && data.loadedFor === session.user.id) return true;

  data.loadedFor = session.user.id;
  data.patient = null; data.assessments = []; data.scores = []; data.oldScores = [];
  data.priorities = []; data.trajectory = []; data.appointments = [];
  ...
  data.oldScores = oldScoreRes.data || [];
  ...
  data.patient = patientRes.data;

  const assessmentsRes = await sb.from('assessments').select('*').eq('patient_id', data.patient.id).order('created_at', { ascending:false }).limit(12);
  data.assessments = assessmentsRes.data || [];
  const ids = data.assessments.map(a => a.id).filter(Boolean);
  const latest = data.assessments[0];

  const requests = [
    sb.from('trajectory_events').select('*').eq('patient_id', data.patient.id).order('event_date', { ascending:false }).limit(16),
    sb.from('organization_appointments').select('*').eq('patient_id', data.patient.id).order('scheduled_start', { ascending:true }).limit(40)
  ];
  if (ids.length) requests.push(sb.from('scores').select('*').in('assessment_id', ids).order('calculated_at', { ascending:false }));
  if (latest?.id) requests.push(sb.from('priorities').select('*').eq('assessment_id', latest.id).order('rank', { ascending:true }));

  const results = await Promise.all(requests);
  data.trajectory = results[0]?.data || [];
  data.appointments = results[1]?.data || [];
  if (ids.length) data.scores = results[2]?.data || [];
  if (latest?.id) data.priorities = results[ids.length ? 3 : 2]?.data || [];
  return true;
}
...
function patchNavigation() {
  document.querySelectorAll('#desktopNav .nav-item, #mobileNav .nav-item').forEach(button => {
    ...
    if (spec.icon) {
      ...
      if (svg && svg.outerHTML !== spec.icon) svg.outerHTML = spec.icon;
    }
  ...
  if (desktop && !desktop.querySelector('[data-route="plan"]')) {
    ...
    desktop.insertBefore(plan, agenda || desktop.querySelector('[data-route="explore"]'));
  }
  ...
  if (mobile) {
    if (!mobile.querySelector('[data-route="plan"]')) {
      const holder = document.createElement('div');
      holder.innerHTML = navButton('plan', 'Mon plan', planIcon);
      mobile.appendChild(holder.firstElementChild);
    }
...
function renderTests() {
  ...
  const readiness = latest?.completeness !== null && latest?.completeness !== undefined ? `${Math.round(Number(latest.completeness))}% complété` : status;
...
function renderMyKomo() {
  ...
  const timeline = data.trajectory.slice(0,8);
  const history = data.scores.length ? data.scores.slice(0,5) : data.oldScores.slice(0,5);
...
function nextAppointmentText() {
  const now = Date.now();
  const next = data.appointments.find(a => a.scheduled_start && new Date(a.scheduled_start).getTime() >= now && !['cancelled','completed','no_show'].includes(a.status));
...
function renderAppointments() {
  const now = Date.now();
  const upcoming = data.appointments.filter(a => a.scheduled_start && new Date(a.scheduled_start).getTime() >= now && !['cancelled','completed','no_show'].includes(a.status));
  const past = data.appointments.filter(a => !a.scheduled_start || new Date(a.scheduled_start).getTime() < now || ['completed','no_show'].includes(a.status)).slice().reverse();
...
document.addEventListener('click', event => {
  ...
  if (refresh) setTimeout(() => scheduleRender(true), 50);
});

const observer = new MutationObserver(() => {
  patchNavigation();
  const shell = document.querySelector('#appShell');
  if (shell && !shell.hidden) scheduleRender(false);
});
...
setTimeout(() => scheduleRender(false), 120);
