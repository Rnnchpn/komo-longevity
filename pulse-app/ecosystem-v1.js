...
function applySignupIntent(){const auth=document.querySelector('#authScreen');if(!auth||auth.hidden)return false;const heading=auth.querySelector('.auth-heading');if(!heading)return false;const pill=heading.querySelector('.product-pill'),h2=heading.querySelector('h2'),p=heading.querySelector('p');if(pill)pill.textContent='Pulse';if(h2)h2.textContent='Créer votre espace Pulse';if(p)p.textContent='Créez votre compte personnel pour commencer vos tests et retrouver votre suivi KŌMØ.';document.querySelector('#emailInput')?.focus();document.querySelector('#signupButton')?.classList.add('signup-intent');return true}
function entry(){const u=new URL(location.href),mode=u.searchParams.get('mode'),intent=u.searchParams.get('intent'),e=u.searchParams.get('entry');if(mode==='professional'){sessionStorage.setItem('komo_pulse_pro_intent','1');u.searchParams.delete('mode');if(!e)u.searchParams.set('entry','clinical')}if(intent==='signup'){applySignupIntent();u.searchParams.delete('intent')}const resolved=u.searchParams.get('entry');if(resolved){const map={home:'home',tests:'results',mykomo:'path',plan:'plan',agenda:'documents',clinical:'clinical',motion:'clinical',professional:'clinical'};const r=map[resolved];if(r&&location.hash!==`#${r}`)location.hash=r;u.searchParams.delete('entry')}history.replaceState({},'',u.pathname+u.search+location.hash)}
const obs=new MutationObserver(()=>requestAnimationFrame(()=>{mount();const u=new URL(location.href);if(u.searchParams.get('intent')==='signup')applySignupIntent()}));obs.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden']});window.addEventListener('resize',mount);document.addEventListener('DOMContentLoaded',()=>{setTimeout(()=>{mount();entry()},500)});setTimeout(()=>{mount();entry()},1000);