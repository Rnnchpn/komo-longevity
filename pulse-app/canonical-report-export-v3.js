...
export async function exportCanonicalMobilityReport({button=null}={}){if(busy)return;busy=true;const old=button?.textContent;try{if(button){button.disabled=true;button.textContent='Préparation du nouveau Mobility Report…'}const snapshot=await officialSnapshot();if(snapshot){await downloadMobilityReport(snapshot.payload,{draft:false});toast(`Mobility Report Luxury officiel v${snapshot.version} téléchargé.`);return}const payload=await currentPayload();await downloadMobilityReport(payload,{draft:true});toast('Mobility Report Luxury actualisé téléchargé en aperçu.')}catch(e){console.error('[canonical-report-export-v3]',e);toast(`Export impossible : ${e?.message||e}`);throw e}finally{busy=false;if(button){button.disabled=false;button.textContent=old||'Télécharger le PDF'}}}
...
window.KomoCanonicalReport={version:VERSION,owner:'mobility-report-luxury-v3',export:exportCanonicalMobilityReport};