...
const REFERENCE='Hawkins et al. / Journals of Gerontology Series A 2017 · Physical Performance Across the Adult Life Span';
...
function equivalentAge(value,key,table){
  ...
  for(const row of table){
    const ref=row[key];if(!ref||!ref.sd)continue;
    const z=Math.abs(value-ref.mean)/ref.sd;
    if(!best||z<best.z)best={age:row.mid,band:[row.min,row.max],mean:ref.mean,sd:ref.sd,z};
  }
  return best;
}
function confidenceFrom(spread,count,sexSpecific){
  let score=count===3?0.82:0.68;
  if(spread<=10)score+=0.12;else if(spread<=20)score+=0.04;else if(spread>25)score-=0.16;
  if(!sexSpecific)score-=0.08;
...
export function computeLocomotorAge(dossier,interpretation={}){
  ...
  const chronologicalAge=ageYears(patientBirth(p));
  const sex=sexKey(patientSex(p));
  const table=REF[sex]||REF.total;
  ...
  const gait=num(measurement(dossier,'M-FUN-05')?.numeric_value);
  const chair=num(measurement(dossier,'M-FUN-04')?.numeric_value);
  const slsL=num(measurement(dossier,'M-FUN-06')?.numeric_value);
  const slsR=num(measurement(dossier,'M-FUN-07')?.numeric_value);
  const sls=(slsL!==null&&slsR!==null)?(slsL+slsR)/2:(slsL??slsR);
  const reasons=[];
  const values=[];
  if(gait!==null&&gait>=0.3&&gait<=2.2)values.push({id:'gait_speed',label:'Vitesse de marche habituelle',value:gait,unit:'m/s',weight:0.36,...equivalentAge(gait,'gait',table)});else reasons.push('Vitesse de marche habituelle standardisée manquante ou hors plage plausible.');
  if(chair!==null&&chair>=1&&chair<=45)values.push({id:'chair_stand_30s',label:'Chair Stand 30 s',value:chair,unit:'répétitions',weight:0.34,...equivalentAge(chair,'chair',table)});else reasons.push('Chair Stand 30 s standardisé manquant ou hors plage plausible.');
  if(sls!==null&&sls>=0&&sls<=60)values.push({id:'single_leg_stance',label:'Appui unipodal',value:Math.round(sls*10)/10,unit:'s',weight:0.30,...equivalentAge(sls,'sls',table)});else reasons.push('Appui unipodal standardisé manquant ou hors plage plausible.');
  if(values.length<2)return notCalculable('Au moins deux des trois tests standardisés sont nécessaires : marche, Chair Stand 30 s, appui unipodal.',{chronologicalAge,sex,inputs:values,reasons});
  ...
  const weighted=values.reduce((s,v)=>s+v.age*v.weight,0)/totalWeight;
  const med=median(ages);
  const age=Math.round((weighted*0.65+med*0.35));
  const confidence=confidenceFrom(spread,values.length,sex!=='total');
  const half=confidence>=0.82?7:confidence>=0.65?10:15;
  const interval=[clamp(age-half,30,99),clamp(age+half,30,99)];
  const deltaYears=age-chronologicalAge;
  const band=table.find(x=>age>=x.min&&age<=x.max)||table[table.length-1];
  return{
    version:VERSION,
    status:'available',
    experimental:true,
    age,
    ageBand:[band.min,band.max],
    interval,
    chronologicalAge,
    deltaYears,
    ...
    inputs:values,
    reasons,
    interpretation:deltaYears<=-8?'Performance locomotrice globalement plus jeune que l’âge chronologique.':deltaYears>=8?'Performance locomotrice globalement plus âgée que l’âge chronologique.':'Performance locomotrice globalement proche de l’âge chronologique.',
    disclaimer:'Estimation expérimentale de performance fonctionnelle, non validée comme biomarqueur d’âge biologique et non destinée à poser un diagnostic.',
    source:{title:REFERENCE,url:REFERENCE_URL,cohort:'775 adultes, 30–90+ ans',metrics:['vitesse de marche habituelle','Chair Stand 30 s','appui unipodal']}
  };
}
