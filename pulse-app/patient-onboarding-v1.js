...
let client=null,handoffShown=false;
...
function modal(){let m=document.querySelector('#patientCreateModal');if(m)return m;m=document.createElement('div');m.id='patientCreateModal';m.className='patient-create-modal';m.hidden=true;document.body.appendChild(m);return m}
function clearStart(){sessionStorage.removeItem('komo_start_check_after_signup');const u=new URL(location.href);u.searchParams.delete('start');history.replaceState({},'',u.pathname+(u.search?u.search:'')+u.hash)}
function fields(){return `<div class="patient-create-grid"><label><span>Prénom *</span><input name="first_name" autocomplete="given-name" required></label><label><span>Nom *</span><input name="last_name" autocomplete="family-name" required></label><label><span>Date de naissance *</span><input name="birth_date" type="date" autocomplete="bday" required></label><label><span>Téléphone *</span><input name="phone" type="tel" autocomplete="tel" required></label><label class="wide"><span>Adresse *</span><input name="address_line1" autocomplete="street-address" required placeholder="Numéro et voie"></label><label><span>Code postal *</span><input name="postal_code" autocomplete="postal-code" required></label><label><span>Ville *</span><input name="city" autocomplete="address-level2" required></label><label><span>Pays *</span><input name="country" autocomplete="country-name" required value="France"></label><label class="wide"><span>Adresse e-mail *</span><input name="email" type="email" autocomplete="email" required></label><label class="wide"><span>Mot de passe *</span><input name="password" type="password" autocomplete="new-password" minlength="6" required></label></div>`}
function open(){handoffShown=false;const m=modal();delete m.dataset.handoff;m.hidden=false;m.innerHTML=`<div class="patient-create-backdrop" data-patient-create-close></div><section class="patient-create-sheet" role="dialog" aria-modal="true" aria-labelledby="patientCreateTitle"><button type="button" class="patient-create-close" data-patient-create-close>×</button><p class="eyebrow">KŌMØ PULSE</p><h2 id="patientCreateTitle">Créer votre espace patient.</h2><p class="patient-create-lead">Quelques informations pour créer votre profil et préparer votre première trajectoire KŌMØ.</p><form id="patientCreateForm">${fields()}<label class="patient-create-consent"><input type="checkbox" required><span>J’accepte la création de mon espace KŌMØ Pulse et le traitement de mes données selon la politique de confidentialité.</span></label><button class="primary-button patient-create-submit" type="submit">Créer mon espace Pulse →</button><p id="patientCreateFeedback" class="patient-create-feedback"></p></form></section>`;m.querySelectorAll('[data-patient-create-close]').forEach(b=>b.addEventListener('click',()=>m.hidden=true));m.querySelector('#patientCreateForm')?.addEventListener('submit',submit)}
function dataFrom(f){const d=new FormData(f);return{first_name:String(d.get('first_name')||'').trim(),last_name:String(d.get('last_name')||'').trim(),birth_date:String(d.get('birth_date')||''),phone:String(d.get('phone')||'').trim(),address_line1:String(d.get('address_line1')||'').trim(),postal_code:String(d.get('postal_code')||'').trim(),city:String(d.get('city')||'').trim(),country:String(d.get('country')||'').trim(),email:String(d.get('email')||'').trim(),password:String(d.get('password')||'')}}
