...
async function refresh(){const shell=document.querySelector('#appShell');if(!shell||shell.hidden)return;if(await isAdmin()){mountTop();mountAccount()}}
function schedule(){clearTimeout(timer);timer=setTimeout(()=>refresh().catch(console.error),100)}
const obs=new MutationObserver(schedule);obs.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['hidden','class']});window.addEventListener('hashchange',schedule);document.addEventListener('DOMContentLoaded',()=>setTimeout(schedule,700));setTimeout(schedule,1200);
