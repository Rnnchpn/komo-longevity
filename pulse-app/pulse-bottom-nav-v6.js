...
function active(){
 ...
 if(['mykomo','profile'].includes(r))return'mykomo';
 return'home';
}
...
#kpDockV6{position:fixed!important;z-index:10000!important;left:50%;bottom:max(10px,env(safe-area-inset-bottom));transform:translateX(-50%);width:min(980px,calc(100vw - 28px));height:70px;padding:5px;display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:3px;border:1px solid rgba(244,241,234,.105);border-radius:23px;background:rgba(12,15,13,.96);box-shadow:0 22px 68px rgba(0,0,0,.40),inset 0 1px rgba(255,255,255,.025);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);isolation:isolate;overflow:hidden;box-sizing:border-box;pointer-events:auto!important;touch-action:manipulation}#kpDockV6[hidden]{display:none!important}
...
@media(max-width:760px){#kpDockV6{left:8px!important;right:8px!important;width:auto!important;height:64px!important;bottom:max(7px,env(safe-area-inset-bottom))!important;transform:none!important;border-radius:21px!important;padding:4px!important}#kpDockV6 .kp6-indicator{top:4px!important;left:4px!important;height:56px!important;border-radius:17px!important}#kpDockV6 a{height:56px!important;padding:2px 1px!important;gap:4px!important}#kpDockV6 .kp6-icon{font-size:15px!important}#kpDockV6 b{font-size:6.4px!important}}
@media(max-width:390px){#kpDockV6 b{font-size:5.7px!important}#kpDockV6 .kp6-icon{font-size:14px!important}}
