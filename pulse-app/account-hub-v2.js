...
function appointmentRows(items,centers){
 if(!items.length)return'<div class="kah-empty kah-appt-empty">Aucune consultation à venir. Vous pouvez réserver votre prochain rendez-vous depuis l’espace Consultations & rendez-vous.<a class="kah-inline-route" href="#documents">Réserver un rendez-vous →</a></div>';
...
function bind(section,current){section.querySelectorAll('[data-kah-cancel]').forEach(b=>b.addEventListener('click',()=>cancelAppointment(b.dataset.kahCancel,b,current)))}
