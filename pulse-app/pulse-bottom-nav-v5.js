/* KŌMØ Pulse — canonical patient dock v5.0.0
...
const active=()=>route()==='motion'?'assessment':route()==='mykomo'?'mykomo':route()==='trajectory'?'trajectory':route()==='documents'?'agenda':'home';
...
#kpDockV5{position:fixed;z-index:190;left:50%;bottom:max(10px,env(safe-area-inset-bottom));transform:translateX(-50%);width:min(840px,calc(100vw - 28px));height:72px;padding:6px;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:3px;border:1px solid rgba(255,255,255,.1);border-radius:24px;background:linear-gradient(145deg,#203027,#17231c);box-shadow:0 18px 56px rgba(18,28,21,.25);isolation:isolate;overflow:hidden;box-sizing:border-box}#kpDockV5[hidden]{display:none!important}
