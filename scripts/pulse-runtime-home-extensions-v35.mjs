...
const homeV3=commandSource.includes("const VERSION='3.0.0'")||commandSource.includes('data-khome-v3');

if(homeV3){
  // Home V3 intentionally owns its complete presentation. The historical KEY,
  // micro-motion and hero overlays target the retired Home markup and must not
  // be re-injected or merged into the new single-layer renderer.
  ...
  for(const file of [micro,key,hero])if(finalHtml.includes(file))throw new Error(`[pulse-home-v35] retired Home V3 extension remains direct: ${file}`);
  if(!commandSource.includes('komo:home-command-rendered'))throw new Error('[pulse-home-v35] Home V3 render event missing');
  if(!commandSource.includes('KomoAssistantV2'))throw new Error('[pulse-home-v35] Home V3 Komo assistant bridge missing');
  console.log('[pulse-home-v35] Home V3 detected · legacy KEY/micro-motion/hero overlays retired · single-layer Home preserved');
}else{
