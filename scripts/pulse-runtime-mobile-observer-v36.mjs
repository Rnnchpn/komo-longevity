...
const utilityReplacement=`viewObserver = {shared:true};\n  window.addEventListener('komo:mobile-dom-change',refresh);`;
...
const guidedReplacement=`viewObserver={shared:true};\n    window.addEventListener('komo:mobile-dom-change',refresh);`;
...
const vertical=`  function bindObservers(){\n    if(appObserver)return;\n    window.addEventListener('komo:mobile-dom-change',()=>schedule(24));\n    appObserver={shared:true};\n    viewObserver=appObserver;\n    authObserver=appObserver;\n  }\n`;
source=source.slice(0,verticalStart)+vertical+source.slice(verticalEnd+1);

const dispatcher=`(()=>{\n  let observer=null;\n  function bindMobileObserver(){\n    const application=document.querySelector('#appShell');\n    if(!application||observer)return;\n    observer=new MutationObserver(mutations=>{\n      if(!mutations.some(m=>m.type==='childList'||m.target===application))return;\n      window.dispatchEvent(new CustomEvent('komo:mobile-dom-change'));\n    });\n    observer.observe(application,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden','class']});\n  }\n  document.addEventListener('DOMContentLoaded',bindMobileObserver,{once:true});\n  ['komo:session-ready','komo:session-cleared','pageshow'].forEach(name=>window.addEventListener(name,bindMobileObserver));\n  bindMobileObserver();\n})();\n`;
