import {readFile} from 'node:fs/promises';import {spawnSync} from 'node:child_process';import {dirname,join} from 'node:path';import {fileURLToPath} from 'node:url';
...
const html=await readFile(join(root,'pulse-app','index.html'),'utf8'),app=await readFile(join(root,'pulse-app','app.js'),'utf8'),runtime=await readFile(join(root,'pulse-app','performance-runtime-v1.js'),'utf8');
...
const checks=[
 ...
 ['Pro architecture is event driven',entries.find(([f])=>f==='pro-architecture-v2.js')[1].includes("addEventListener('komo:route-ready'"))],
 ['booking is event driven',entries.find(([f])=>f==='booking-layer-v1.js')[1].includes("addEventListener('komo:route-ready'"))],
 ['messaging is event driven',entries.find(([f])=>f==='care-messaging-v1.js')[1].includes("addEventListener('komo:route-ready'"))],
 ['auth gateway no longer tracks DOM mutations',entries.find(([f])=>f==='auth-gateway-v2.js')[1].includes("addEventListener('komo:session-ready'"))]
];
const failed=checks.filter(([,ok])=>!ok).map(([n])=>n);if(failed.length){for(const [f,r] of syntax)if(r.status!==0)console.error(`[${f}] ${r.stderr||r.stdout}`);console.error('[pulse-performance-qa-v1] failed: '+failed.join(', '));process.exit(1)}console.log(`[pulse-performance-qa-v1] ${checks.length} checks passed.`);
