...
const checks=[
 ...
 ['admin console role gated',consoleJs.includes("state.role !== 'admin'")&&consoleJs.includes("from('account_roles')")],
 ['admin shell renders before data',consoleJs.includes('Ouverture de la console Admin')&&consoleJs.includes('renderFrame();')],
 ['backend failures are isolated',consoleJs.includes('Promise.allSettled')&&consoleJs.includes('state.errors.patients')&&consoleJs.includes('state.errors.pros')&&consoleJs.includes('state.errors.motion')],
...
if(failed.length){
  ...
  console.error('[admin-console-qa-v2] failed: '+failed.join(', '));
  process.exit(1);
}
