...
for(const c of cfg){for(const f of await walk(c.dir)){let h=await readFile(f,'utf8');if(!h.includes('class="footerread"'))continue;h=h.replace(/<style id="media-product-bridge-style">[\s\S]*?<\/style>/g,'');h=h.replace('</head>',`${style}</head>`);if(!h.includes('media-motion-link'))h=h.replace(/(<section class="footerread"[\s\S]*?<div class="footerin">[\s\S]*?)(<\/div><\/section>)/,`$1<a class="media-motion-link" href="${c.motion}">${c.label}</a>$2`);await writeFile(f,h,'utf8')}}
