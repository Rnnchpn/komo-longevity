...
const currentFn=`function currentRoute(){let route=location.hash.replace(/^#/,'')||'home';if(route==='followup'||route==='suivi')route='key';if(route==='connected')route='link';if(route==='clinical'&&!['professional','admin'].includes(state.role))return'home';return['home','results','path','documents','explore','clinical','profile','motion','mykomo','club','key','trajectory','link'].includes(route)?route:'home'}\n`;
...
const delegated="function renderRoute(route){renderNavigation();if(['motion','mykomo','club','key','trajectory','link'].includes(route)){window.dispatchEvent(new CustomEvent('komo:route-ready',{detail:{route,source:'app-external-owner'}}));return}";
...
console.log('[pulse-route-v2] canonical ownership fixed for Motion · My KŌMØ · Club · KEY · Trajectoire · Link; Agenda preserved');
