...
const agenda=await patch('site/pulse-v12/agenda-hub-v4.js',[["window.addEventListener('hashchange',()=>{if(route()==='documents')setTimeout(()=>refresh(),30);else document.body.classList.remove('agenda-v4')});window.addEventListener('pageshow',()=>{if(route()==='documents')setTimeout(()=>refresh(),70)});document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{if(route()==='documents')refresh()},500));","window.addEventListener('hashchange',()=>{if(route()==='documents')setTimeout(()=>refresh(),30);else document.body.classList.remove('agenda-v4')});window.addEventListener('pageshow',()=>{if(route()==='documents')setTimeout(()=>refresh(),70)});window.addEventListener('komo:route-ready',()=>{if(route()==='documents')setTimeout(()=>refresh(),30)});window.addEventListener('komo:session-ready',()=>{if(route()==='documents')setTimeout(()=>refresh(),30)});document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{if(route()==='documents')refresh()},500));"]],'Agenda owner');
