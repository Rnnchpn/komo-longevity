...
const release='20260829-home-key-position-v2';
...
const checks=[
 ...
 ['KEY renderer is awaited after datawall rebuild',js.includes('await window.KomoKeyHome.refresh()')],
 ...
 ['placement has no MutationObserver or permanent polling',!js.includes('MutationObserver')&&!js.includes('setInterval(')]
];
for(const [label,ok] of checks) console.log(`[pulse-home-key-position-v2] ${ok?'OK':'FAIL'} · ${label}`);
if(checks.some(([,ok])=>!ok)) process.exit(1);
console.log('[pulse-home-key-position-v2] PASS · Motion Score + Locomotor Age first, KEY immediately below after async home rebuilds');
