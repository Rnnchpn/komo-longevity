...
const release='20260831-patient-v1-fixpack-1';
const navRelease='20260831-patient-nav-v702';
...
for(const file of [cssFile,heroCssFile,dailyCssFile,mobileCssFile,jsFile,dailyJsFile,mobileJsFile,aiClientFile,assistantJsFile,assistantCssFile,resultsOwnerFile,agendaMapFile,navFile]){
...
for(const file of [entryFile,jsFile,dailyJsFile,mobileJsFile,aiClientFile,assistantJsFile,resultsOwnerFile,agendaMapFile,clarityJsFile]){
  const escaped=file.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
...
const [css,heroCss,dailyCss,mobileCss,js,dailyJs,mobileJs,aiClient,assistantJs,assistantCss,resultsOwner,agendaMap,nav,final]=await Promise.all([
...
const checks=[
  ...
  ['Home greeting has patient accent color',css.includes('--kh3-title:#b8c9bb')&&css.includes('color:var(--kh3-title)')],
  ...
  ['imported runtimes are not direct scripts',requiredImports.every(file=>!directScript(file))&&!directScript(dailyJsFile)],
  ...
  ['Komo has safe Pulse fallback',aiClient.includes('safeFallback')&&aiClient.includes("action:'overview'")&&aiClient.includes('fallback:true')],
...
console.log('[pulse-home-web-v1] PASS · patient V1 fixpack · Home/nav/Results/map/Komo');