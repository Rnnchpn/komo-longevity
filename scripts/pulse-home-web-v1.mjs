...
const checks=[
  ...
  ['final Home V4 CSS loaded',final.includes(`${cssFile}?v=${release}`)],
  ['Home greeting is forest green and readable',css.includes('.kh3-head h2')&&css.includes('color:#234331!important')],
  ['Home identifies KŌMØ immediately',js.includes('kh3-brand-rail')&&js.includes('KŌMØ PULSE')&&css.includes('.kh3-brand-rail')],
  ['Home is a fixed one-screen Today surface',css.includes('height:100dvh!important')&&css.includes('overflow:hidden!important')&&css.includes('.kh3-movement')&&css.includes('.kh3-strip')&&css.includes('.kh3-next')],
  ['Home exclusively owns its DOM',js.includes('host.replaceChildren(node)')&&js.includes("host.dataset.khomeOwner='v4'")],
  ...
  ['current Home uses dynamic Today surfaces',js.includes('VOTRE JOURNÉE EN MOUVEMENT')&&js.includes('KEY · VOTRE QUOTIDIEN')&&js.includes('KŌMØ WALK CLUB')&&js.includes('PROCHAINE CONSULTATION')],
  ...
  ['Home keeps clinical scoring out of the daily surface',!js.includes('loadCanonicalResult')&&!js.includes('Motion Score')&&!js.includes("from('scores')")],
  ['Home uses canonical consent-aware walk summary',js.includes("rpc('komo_walk_summary')")&&!js.includes("from('wearable_daily_metrics')")],
  ['Home turns KEY into one contextual daily insight',js.includes('keyInsight(walk')&&js.includes('moyenne des 7 derniers jours')&&js.includes('couverture KEY')],
  ...
  ['Home exposes essential daily destinations',js.includes("route:'key'")&&js.includes("route:'club'")&&js.includes("route:'documents'")],
  ['Home exposes steps K Points and Walk Club',js.includes('steps_today')&&js.includes('k_points_today')&&js.includes('k_points_week')&&js.includes('KŌMØ WALK CLUB')],
...
console.log('[pulse-home-web-v1] PASS · dynamic Today Home · KEY · Walk Club · K Points · consultation · Komo');
