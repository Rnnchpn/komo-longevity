...
for(const file of ['pulse-iphone-density-v1.css','pulse-wearable-mobile-v3.css','pulse-iphone-no-topbar-v1.css','pulse-watch-extension-mobile-v4.css']){
...
const [css,wearable,noTopbar,watch,final]=await Promise.all([
...
const checks=[
  ...
  ['watch-extension is final phone presentation owner',final.includes(`pulse-watch-extension-mobile-v4.css?v=${watchRelease}`)&&final.indexOf(`pulse-watch-extension-mobile-v4.css?v=${watchRelease}`)>final.indexOf(`pulse-iphone-no-topbar-v1.css?v=${noTopbarRelease}`)],
  ...
  ['layout remains phone-only',css.includes('@media (max-width:767px)')&&wearable.includes('@media (max-width:767px)')&&noTopbar.includes('@media (max-width:767px)')&&watch.includes('@media (max-width:767px)')],
  ['no auth visibility ownership',![wearable,noTopbar,watch].some(x=>x.includes('#authScreen[hidden]')||x.includes('#appShell[hidden]'))],
  ['no score/data behavior in visual layers',![wearable,noTopbar,watch].some(x=>x.includes('fetch(')||x.includes('supabase')||x.includes('motion_score='))]
];
for(const [label,ok] of checks)console.log(`[pulse-watch-extension-mobile-v4] ${ok?'OK':'FAIL'} · ${label}`);
if(checks.some(([,ok])=>!ok))process.exit(1);
console.log('[pulse-watch-extension-mobile-v4] PASS · KEY-first phone home · Motion/Age context · one persistent tab bar');
