...
for(const p of await walk(site)){const h=await readFile(p,'utf8');for(const bad of ['/_vercel/insights/script.js','/fr/confidentialite/','/fr/mentions-legales/'])if(h.includes(bad))failures.push(`${p.replace(site,'site')} contains stale ${bad}`)}
