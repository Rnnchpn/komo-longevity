...
for(const file of [...reachable].sort()){
  ...
  metrics.push({file,bytes:st.size,observers,bodyObservers,intervals,timeouts,createClients,routeWrites,viewWrites,globals});
}
...
function score(m){return m.observers*8+m.bodyObservers*12+m.intervals*10+m.routeWrites*5+m.viewWrites*6+m.createClients*5+Math.min(8,Math.floor(m.timeouts/4));}
...
const domObservers=metrics.filter(x=>x.observers>0);
...
console.log(`[pulse-runtime-debt-v1] loaded bytes=${metrics.reduce((a,b)=>a+b.bytes,0)} · MutationObserver=${metrics.reduce((a,b)=>a+b.observers,0)} · whole-body observers=${wholeBody.length} · intervals=${metrics.reduce((a,b)=>a+b.intervals,0)} · createClient=${directClients.length} modules · route writers=${routeWriters.length} · view writers=${viewOwners.length}`);
...
for(const m of ranked.slice(0,20))console.log(`  ${String(m.risk).padStart(3)} · ${m.file} · obs=${m.observers}/${m.bodyObservers} int=${m.intervals} timeout=${m.timeouts} sb=${m.createClients} route=${m.routeWrites} view=${m.viewWrites} bytes=${m.bytes}`);
console.log('[pulse-runtime-debt-v1] direct Supabase clients',directClients.map(x=>x.file).join(', ')||'none');
...
const report={generated_at:new Date().toISOString(),direct_scripts:scripts.length,unique_script_tags:scriptCounts.size,reachable_modules:reachable.size,total_js_modules:jsFiles.length,loaded_bytes:metrics.reduce((a,b)=>a+b.bytes,0),mutation_observers:metrics.reduce((a,b)=>a+b.observers,0),whole_body_observers:wholeBody.map(x=>x.file),interval_modules:metrics.filter(x=>x.intervals>0).map(x=>({file:x.file,count:x.intervals})),direct_supabase_clients:directClients.map(x=>x.file),route_writers:routeWriters.map(x=>x.file),view_writers:viewOwners.map(x=>x.file),duplicate_script_tags:duplicateScriptTags,top_risk:ranked.slice(0,30),surface_candidates:Object.fromEntries(Object.entries(owners).map(([k,v])=>[k,v.filter(x=>reachable.has(x))]))};
console.log('[pulse-runtime-debt-v1] REPORT_JSON '+JSON.stringify(report));

// Hard guards only for regressions that should never be tolerated.
if(duplicateScriptTags.length){console.error('[pulse-runtime-debt-v1] FAILED · duplicate direct script tags detected');process.exit(1)}
if(scripts.length>130){console.error(`[pulse-runtime-debt-v1] FAILED · direct script count exploded to ${scripts.length}`);process.exit(1)}
console.log('[pulse-runtime-debt-v1] PASS · inventory produced; no deletion performed');
