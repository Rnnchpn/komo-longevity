...
Deno.serve(async(req:Request)=>{
  ...
  const admin=async()=>await role()==="admin";
  ...
  if(action==="submit"){
    ...
    if(!["data_export","account_closure"].includes(type))return json(req,{error:"invalid_request_type"},400);
    const note=String(body.request_note??"").trim().slice(0,1500)||null;
    const open=await svc.from("account_privacy_requests").select("id,request_type,status,requested_at").eq("user_id",user.id).eq("request_type",type).in("status",["submitted","in_review"]).order("requested_at",{ascending:false}).limit(1).maybeSingle();
    ...
    const ins=await svc.from("account_privacy_requests").insert({user_id:user.id,request_type:type,status:"submitted",request_note:note}).select("id,request_type,status,requested_at").single();
    if(ins.error){
      if(String(ins.error.code)==="23505"){
        const retry=await svc.from("account_privacy_requests").select("id,request_type,status,requested_at").eq("user_id",user.id).eq("request_type",type).in("status",["submitted","in_review"]).order("requested_at",{ascending:false}).limit(1).maybeSingle();
    ...
    const label=type==="data_export"?"copie des données":"fermeture du compte";
    await Promise.allSettled([
      mail(user.email,"Votre demande KŌMØ Pulse a été enregistrée",`<p>Votre demande de <strong>${label}</strong> a bien été enregistrée.</p><p>Elle est tracée dans votre espace Compte.</p>`),
      mail(ADMIN,"Nouvelle demande confidentialité · KŌMØ Pulse",`<p>Nouvelle demande de <strong>${safe(label)}</strong>.</p><p>Référence : <strong>${safe(ins.data.id)}</strong></p><p>Aucune donnée de santé n’est incluse.</p>`)
    ]);
  ...
  if(action==="list_admin"){
    if(!await admin())return json(req,{error:"admin_required"},403);
    ...
    const rows=await Promise.all((q.data??[]).map(async(x:any)=>{
      ...
      return{...x,display_name:display,email:a.data?.user?.email??null,access_control:c.data??null};
    }));
  ...
  if(action==="admin_update"){
    if(!await admin())return json(req,{error:"admin_required"},403);
    const id=String(body.request_id??""),next=String(body.status??"");
    ...
    const current=lookup.data;if(!current)return json(req,{error:"request_not_found"},404);
    if(current.request_type==="account_closure"&&next==="completed")return json(req,{error:"closure_requires_execution_action",detail:"Utilisez l’action serveur de fermeture."},409);
    ...
    const now=new Date().toISOString(),terminal=["completed","declined"].includes(next);
    ...
    if(!up.data)return json(req,{error:"privacy_request_changed",detail:"Rechargez la file."},409);
    ...
    const msg=next==="in_review"?`<p>Votre demande concernant la <strong>${label}</strong> est prise en charge par l’équipe KŌMØ.</p>`:next==="completed"?`<p>Votre demande concernant la <strong>${label}</strong> est indiquée comme traitée.</p>`:`<p>Votre demande concernant la <strong>${label}</strong> est clôturée. Contactez KŌMØ si nécessaire.</p>`;
    await mail(target.data?.user?.email,"Mise à jour de votre demande · KŌMØ Pulse",msg);
    return json(req,{ok:true,request:up.data});
  }
  ...
  if(action==="admin_execute_closure"){
    ...
    if(String(body.closure_confirmation??"")!=="FERMER")return json(req,{error:"closure_confirmation_required",detail:"La confirmation FERMER est requise."},400);
    ...
    const control=await svc.from("account_access_controls").select("user_id,status,privacy_request_id,last_error,closed_at").eq("user_id",current.user_id).maybeSingle();
    if(control.error)return json(req,{error:"access_control_lookup_failed",detail:control.error.message},500);
    if(control.data?.status==="closed"&&control.data?.privacy_request_id===id){
      if(current.status!=="completed"){
        const t=new Date().toISOString();
        await svc.from("account_privacy_requests").update({status:"completed",resolution_note:note,handled_by:user.id,updated_at:t,resolved_at:t}).eq("id",id);
      }
      return json(req,{ok:true,idempotent:true,status:"closed",closed_at:control.data.closed_at});
    }
    if(current.status!=="in_review")return json(req,{error:"closure_requires_review",detail:"La demande doit être en cours de traitement."},409);

    const target=await svc.auth.admin.getUserById(current.user_id);
    const targetEmail=target.data?.user?.email??null;
    const missing=!!target.error&&/not.?found|user_not_found/i.test(`${(target.error as any)?.code??""} ${target.error?.message??""}`);
    const resumable=missing&&control.data?.status==="closing"&&control.data?.privacy_request_id===id&&!control.data?.last_error;
    if(target.error&&!resumable)return json(req,{error:"auth_user_lookup_failed",detail:target.error.message},409);
