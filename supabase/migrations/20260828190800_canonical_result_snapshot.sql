-- KŌMØ Pulse · one result snapshot shared by patient and professional views.
-- Patient mode exposes only released Motion values. Professional mode exposes the current score plus publication state.
