...
begin
  ...
  if motion_row.id is not null then
    ...
    select coalesce(jsonb_agg(jsonb_build_object('id',qs.id,'instrument_code',qs.instrument_code,'instrument_version',qs.instrument_version,'status',qs.status,'score',qs.score,'score_status',qs.score_status,'completeness',qs.completeness,'started_at',qs.started_at,'completed_at',qs.completed_at,'responses',coalesce((select jsonb_agg(jsonb_build_object('item_code',qr.item_code,'response_code',qr.response_code,'raw_value',qr.raw_value,'normalized_value',qr.normalized_value,'source',qr.source,'clinician_verified',qr.clinician_verified,'completed_at',qr.completed_at) order by qr.item_code) from public.questionnaire_responses qr where qr.questionnaire_session_id=qs.id),'[]'::jsonb)) order by qs.created_at),'[]'::jsonb) into q_sessions from public.questionnaire_sessions qs where qs.assessment_id=motion_row.id;
    select coalesce(jsonb_agg(jsonb_build_object('id',m.id,'indicator_code',m.indicator_code,'product_status',m.product_status,'raw_value',m.raw_value,'numeric_value',m.numeric_value,'text_value',m.text_value,'unit',m.unit,'source',m.source,'task_code',m.task_code,'muscle_code',m.muscle_code,'side',m.side,'protocol_version',m.protocol_version,'qc_status',m.qc_status,'qc_reason',m.qc_reason,'recorded_at',m.recorded_at,'source_reference',m.source_reference) order by m.recorded_at,m.indicator_code),'[]'::jsonb) into measurements_json from public.measurements m where m.assessment_id=motion_row.id;
    ...
    select coalesce(jsonb_agg(jsonb_build_object('id',i.id,'external_session_id',i.external_session_id,'source_file_name',i.source_file_name,'source_product',i.source_product,'source_version',i.source_version,'contract_version',i.contract_version,'source_format',i.source_format,'import_hash',i.import_hash,'payload_manifest',i.payload_manifest,'device_set_id',i.device_set_id,'status',i.status,'warnings',i.warnings,'approved_at',i.approved_at,'recorded_at',i.recorded_at,'created_at',i.created_at) order by i.created_at desc),'[]'::jsonb) into imports from public.myodev_imports i where i.assessment_id=motion_row.id;
  end if;
